...
> #####Sequence of invoking behaviors on objects
    ...
    if siva.login(id,passowrd)
    	...
    	if siva wants to see and send notification
    		siva.viewNotification(news)
    		siva.sendNotification(news)
    	if siva wants to grade ben's assignment
    		siva.setAssgnmentGrade(ben)
    	if siva wants to upload learning material named javaBook
    ...
    if sun.login(id,passowrd)
    	if sun wants to view course information
    		siva.viewCourse(iNFO5100)
    	if sun wants to see and send notification
    		sun.viewNotification(news)
    		sun.sendNotification(news)
    	if sun wants to grade ben's assignment
    		sun.setAssgnmentGrade(ben)
    	if sun wants to upload learning material named javaBook
...
- behavior:
login,logout,setInfo,changePassword,searchpet,adoption,askForHelp
...
name,id,password,phoneNumber,email
- behavior:
login,logout,ProcessOrder,setDiscount
    ...
    if ben wants to leave Beijing to Seattle by air:
    	...
    	if ben decide to buy the ticket
    		...
    		ben.pay()
    		unionAirlines.processOrder(beijing2Seattle)
